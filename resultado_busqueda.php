...
<body>
    ...
    <section class="contactar-container">
        ...
        <h2 id="title-index">Resultado de su búsqueda</h2>
...
$sql = "SELECT * FROM tbl_productos WHERE Nombre LIKE '%".$buscar."%'";
...
?>
        ...
        if($buscar == null || $buscar == ""){
            ?> 
                <h2 class="Error"> No hay resultados </h2>
            <?php
        }else{


            $comparar = "SELECT * FROM tbl_productos WHERE Nombre LIKE '%".$buscar."%' LIMIT " . $primera_pagina_resultado . ',' . $resultados_x_pagina;
            $resultadoComparar = mysqli_query($conexion, $comparar);

            $fila = mysqli_num_rows($resultadoComparar);
            ...
            while($row = mysqli_fetch_array($resultadoComparar)){
                ?>
